...
class MainApplication(tk.Tk):
    def __init__(self):
        ...
        bet_input = tk.Entry(bet_frame,
                             width=10,
                             textvariable=bet_value_var,
                             font=("Helvetica", 14),
                             justify=tk.CENTER)
        bet_input.grid(row=0, column=1)
        play_button = tk.Button(bet_frame,
                                text="Play (Submit Bet)",
                                font=("Helvetica", 14))
        ...
        hit_button = tk.Button(command_frame,
                               text="Hit",
                               font=("Helvetica", 14),
                               state="disabled")
        hit_button.grid(row=0, column=0, padx=10)
        double_down_button = tk.Button(command_frame,
                                       text="Double Down",
                                       state="disabled",
                                       font=("Helvetica", 14))
        double_down_button.grid(row=0, column=1, padx=10)
        split_button = tk.Button(command_frame,
                                 text="Split",
                                 font=("Helvetica", 14),
                                 state="disabled")
        split_button.grid(row=0, column=2, padx=10)
        stand_button = tk.Button(command_frame,
                                 text="Stand",
                                 font=("Helvetica", 14),
                                 state="disabled")
    ...
    def define_settings(self):
        ...
        label_num_decks = tk.Label(
            text=f"Number of decks [{min_num_decks}, {max_num_decks}]: ")
        entry_num_decks = tk.Entry(width=entry_width,
                                   textvariable=num_decks_var)
        ...
        label_num_players = tk.Label(
            text=
            f"Number of players (player + computer) [{min_num_players}, {max_num_players}]: "
        )
        entry_num_players = tk.Entry(width=entry_width,
        ...
        label_seat_no = tk.Label(
            text=
            f"Player's seat number [{min_user_seat_no}, number of players]: ")
        entry_seat_no = tk.Entry(width=entry_width,
                                 textvariable=user_seat_no_var)
        ...
        label_starting_money = tk.Label(
            text="Player's starting money [1,1000000000]: ")
        entry_starting_money = tk.Entry(width=entry_width,
                                        textvariable=starting_money_var)
        button_starting_money = tk.Button(text="Submit",
                                          command=set_starting_money)
        ...
        button_start_game = tk.Button(text="Start Game",
                                      height=2,
                                      command=start_game)
